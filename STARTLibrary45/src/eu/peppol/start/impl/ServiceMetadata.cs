...
namespace STARTLibrary.src.eu.peppol.start.impl
{
    ...
    internal class ServiceMetadataPublishing
    {
        ...
        private XmlDocument LoadMetadataXml(string recipientParticipant, string businessIdScheme, string smlDomain, string documentIdScheme, string documentIdValue)
        {
            ...
            try
            {
                var temp = help.BuildSmpSignedServiceUrl(recipientParticipant, businessIdScheme, smlDomain, documentIdScheme, documentIdValue);
                doc.Load(temp);
            }
            catch (System.Net.WebException ex)
            {
                ...
                switch (ex.Status)
                {
                    ...
                    case System.Net.WebExceptionStatus.ConnectFailure:
                        throw new Exception("ConnectionFailure, The endpoint is not known", ex);
                    ...
                    case System.Net.WebExceptionStatus.MessageLengthLimitExceeded:
                        break;
                    case System.Net.WebExceptionStatus.NameResolutionFailure: throw new Exception("NameResoultionFailure, the endpoint is not known", ex);
        ...
        {
            //xml==SignedServiceMetadata,  ServiceMetadata/ServiceInformation/ProcessList/Process/ServiceEndpointList/Endpoint(transportprofile*)/Certificate

            //NOTE: må ha riktig processidentifier på klient og server for at koden under skal gi treff..
            ...
            if (base64EncodedCertificate == null)
                throw new Exception(string.Format("Could not extract certificate from metadata. ProcessIdentifierScheme={0} ProcessIdentifierValue={1}", ProcessIdentifierScheme, ProcessIdentifierValue));

            byte[] decoded = Convert.FromBase64String(base64EncodedCertificate);
